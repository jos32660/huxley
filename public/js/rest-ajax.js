...
function postData(){
	...
	ajax("/rest-ajax", "post", {username: username}, function(){
		if(res.code == 200) getData();
		else alert("통시에 실패하였습니다.");
		$("#username-wr").val('');
	});
}
...
function renderData(res){
	...
	for(var i in res) {
		...
		html += '<button class="bt-del btn btn-danger btn-sm mr-2" onclick="revData(this)">DEL</button>';
		html += '<button class="bt-up btn btn-success btn-sm mr-2" onclick="chgData(this)">CHG</button>';
		html += '</li>';
	}
	$(".lists").html(html);
}